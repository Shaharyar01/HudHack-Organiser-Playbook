# 🚀 HUDHACK 2026: The Official Archive & Playbook

🔗 **Live Website:** [hudhack.com](https://hudhack.com/)

Welcome to the official open-source archive of **HudHack 2026**, the inaugural hackathon organised by the **University of Huddersfield Cyber Security Society**. 

This repository serves two purposes:
1. **The Archive:** A time capsule of our first event, preserving the website code, the projects built, and the challenges solved.
2. **The Playbook:** A comprehensive, step-by-step guide for future committees. We have documented everything we did right, and everything we did wrong, so HudHack 2027 can be even bigger and better.

---

## 📊 The Event at a Glance
* **Date:** Thursday, 26th February 2026
* **Location:** Haslett Building (HA4/04), University of Huddersfield
* **Theme:** Break The Loop (Identifying everyday frustrations and using tech to solve them)
* **Attendees:** 61 Students
* **Projects Built:** 10+
* **Platforms Used:** Devpost (`hudhack2026.devpost.com`) and CTFd (`hudhack.ctfd.io`)

---

## 🗂️ Repository Directory (How to use this playbook)

If you are on the organizing committee for the next HudHack, start by exploring these folders. Each folder contains its own detailed `README.md` tutorial and all the raw assets you need.

### 🌐 [/Websites-and-Sign-Up-Process](./Websites-and-Sign-Up-Process)
* **What's inside:** The source code for the pre-event and post-event websites, the Google Form registration blueprint, and the automated ticketing script (Google Apps Script).
* **Key Lesson:** Read the warning about university spam filters before you send out your acceptance emails!

### 📢 [/Marketing](./Marketing)
* **What's inside:** The Canva posters, digital screen assets, and our overall strategy for getting 60+ attendees to commit.
* **Key Lesson:** Face-to-face lecture shoutouts and massive QR codes are your best friends.

### 🚩 [/CTF-and-Challenges](./CTF-and-Challenges)
* **What's inside:** The full export file of our 2026 CTFd board, advice on dynamic scoring, and container setups. 
* **Key Lesson:** CTFs are incredibly popular. Prepare more challenges than you think you need, and make the first few a basic tutorial.

### 🏆 [/Devpost-and-Judging](./Devpost-and-Judging)
* **What's inside:** The official 30-point judging rubric, Devpost branding assets, and our guide to roaming judging.
* **Key Lesson:** Set strict platform deadlines and scale up to digital spreadsheets for score tallying if the event grows.

### 🎬 [/On-The-Day](./On-The-Day)
* **What's inside:** The HTML/CSS presentations, the Participant Guide, printable table numbers, and Discord QR cards.
* **Key Lesson:** Plan your volunteer roles beforehand, use table numbers to track teams, and keep the timetable on the screens!

---

## 🤝 The 2026 Organizing Team
A massive thank you to the volunteers who built this from the ground up:
* **Lead Organisers:** Ibrahim & Muhammad
* **CTF Designers:** Angelina, Rohail, Ibrahim K. & Laiba
* **Marketing:** Hasan, Grace, Faiz & Laiba
* **Event Operations:** Arnav (Quizzler) & Olu (Volunteer)

## 💼 Our 2026 Partners
This event would not have been possible without the support of our sponsors:
* **Hackathons UK**
* **University of Huddersfield**
* **Eaton Smith Solicitors**

---
*Built by the UoH Cyber Security Society. Break the loop. Start hacking.*
